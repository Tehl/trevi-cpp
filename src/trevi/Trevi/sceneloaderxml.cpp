...
void SceneLoaderXML::Load ( void )
{
	...
	for (list<Element>::iterator iter = cameras.begin(); iter != cameras.end(); iter++)
	{
		...
		// focus
		double focus;
		thisCamera.FirstChild("FocalLength").Content() >> focus;

		// aperture
		double aperture;
		thisCamera.FirstChild("Aperture").Content() >> aperture;

		// occlusion
		int occlusion;
		thisCamera.FirstChild("Occlusion").Content() >> occlusion;

		// diffuse traces
		int diffusetraces;
		thisCamera.FirstChild("DiffuseTraces").Content() >> diffusetraces;

		// diffuse samples
		int diffusesamples;
		thisCamera.FirstChild("DiffuseSamples").Content() >> diffusesamples;

		// build the camera
		Camera* myCamera = new Camera(Vector(px,py,pz), Vector(dx,dy,dz), resx, resy, fov, samples, raydepth, focus, aperture, occlusion, diffusetraces, diffusesamples);
		myCamera->cache();
	...
	for (list<Element>::iterator iter = lights.begin(); iter != lights.end(); iter++)
	{
		...
		// radius
		double radius;
		thisLight.FirstChild("Radius").Content() >> radius;
		...
		if (thisLight.Attribute("type") == "point")
		{
			...
			// build the light
			myLight = new LightPoint ( Vector ( px, py, pz ), Colour ( r, g, b ), radius, falloff );
		}
		else if (thisLight.Attribute("type") == "spot")
		{
			...
			// build the light
			myLight = new LightSpot ( Vector ( px, py, pz ), Colour ( r, g, b ), radius, Vector ( dx, dy, dz ), cutoff, spread );
		}
		else
		{
			// unknown type
	...
	for (list<Element>::iterator iter = meshes.begin(); iter != meshes.end(); iter++)
	{
		...
		if (thisMesh.Attribute("type") == "implicit")
		{
			...
			for (int i = 0; i < numVerts; i++)
			{
				...
				if (invert)
					myMesh->vertices().push_back( Vertex ( Vector ( px, py, pz ), Vector ( -nx, -ny, -nz ).unit(), u, v ) );
				else
					myMesh->vertices().push_back( Vertex ( Vector ( px, py, pz ), Vector ( nx, ny, nz ).unit(), u, v ) );
			}
